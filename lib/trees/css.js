...
const SassSimultaneousCompiler = require('../sass-simultaneous');
let funnel = require('broccoli-funnel');
let PostcssFilter = require('broccoli-postcss');
...
module.exports = function (environmentOptions, buildOptions, vendorFiles) {
  ...
  let compiled = new SassSimultaneousCompiler(sassDirs, inputToOutputMap, sassSettings);

  if (environmentOptions['environment'] === 'production') {
    compiled = new PostcssFilter(compiled, {
      plugins: [
        {
          module: cssnano,
          options: {

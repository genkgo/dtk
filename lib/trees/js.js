...
module.exports = function (environmentOptions, buildOptions, amdModules, vendorFiles) {
  ...
  let vendorJs = funnel(new source.UnwatchedDir(environmentOptions['modulesDir']), {
    'files': vendorJsFiles
  });
  ...
  let linter = lint(environmentOptions['baseDir'] + '/app/assets/js', {
    options: {
      configFile: rcfile
    }
  });

  let appJs = new Transpiler(linter, environmentOptions['name']);

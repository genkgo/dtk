...
DtkApp.prototype.build = function (requestedEnvironmentOptions) {
  ...
  if (environmentOptions['environment'] === 'production' && this.buildOptions['build-xsl'] !== false && environmentOptions['build-xsl'] !== false) {
    buildXsl(environmentOptions['baseDir'] + '/app/templates/build.xsl', this.buildOptions['hash']);
  }

  return mergeTrees([
    appJsTree(environmentOptions, this.buildOptions, vendorFiles, amdModules),
    appCssTree(environmentOptions, this.buildOptions, vendorFiles),
    assetsTree(environmentOptions, this.buildOptions, vendorFiles),
    copyTree(environmentOptions, this.buildOptions, vendorFiles),
  ]);
};

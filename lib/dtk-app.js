...
DtkApp.prototype.build = function (requestedEnvironmentOptions) {
  ...
  let mergedTree = mergeTrees([appJs, appCss, assets, templates]);

  if (environmentOptions['environment'] === 'production' && usesGit(environmentOptions['baseDir'])) {
    mergedTree = mergedTree.afterBuild(tree, () => {
      shell.exec('git add public/build');
    });
  }
